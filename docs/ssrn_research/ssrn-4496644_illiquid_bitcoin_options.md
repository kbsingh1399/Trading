...
- **Source File**: `ssrn-4496644.pdf`
- **Total Pages**: 50
- **SSRN ID**: `ssrn-4496644`
...
1
This preprint research paper has not been peer reviewed. Electronic copy available at: https://ssrn.com/abstract=4496644
...
The CFTC-approved Bitcoin options contracts are traded on the LedgerX exchange as a
“dealers-driven” market – that is, designated liquidity providers simultaneously post bid and
ask prices waiting for prospective investors. We gather transaction data from the exchange
and present several findings of this CFTC-approved options market:
...
offerings to existing customers (indeed, it continues operation under CFTC regulation after
FTX’s collapse, and is not part of FTX’s and FTX.US’s related bankruptcy cases), and we
will continue to refer to the exchange as LedgerX throughout the paper.
...
month, the exchange may list additional contracts for the current month. In the second half
of the current month, LedgerX may list additional contracts for the current month and the
following month. For weekly contracts, the exchange will introduce a strike at the At-The-
Money (ATM) level, and at minimum two strikes above and two strikes below the ATM level
in $1,000 strike intervals. The exchange will add additional strikes in accordance with the
underlying price movement using 1000, 2000, 4000 strike increments. Weekly strikes may be
listed up to 20% above the ATM strike level and down to 20% below the ATM strike level,
in accordance with BTC price volatility.
2
Data
...
Table A2: Options Liquidity and Returns
Panel A: All Options
DEPENDENT VARIABLE
Delta-Hedged Return
(1)
...
0.041
0.057
This table further demonstrates the robustness of the main findings in Table 5. We conduct a similar
analysis as in Table 5, but with the imbalance variable substituted by a dummy variable (with +1
indicating net buying pressure from clients, 0 indicating a balanced market, and −1 indicating net selling
pressure from clients). Robust t-statistics are in parentheses with standard errors clustered at the contract
level. Stars denotes p-statistics < 0.01, < 0.05, and < 0.1, respectively.
44
This preprint research paper has not been peer reviewed. Electronic copy available at: https://ssrn.com/abstract=4496644
Preprint not peer reviewed
...
Table A3: Options Liquidity and Returns before and after the policy shock
Panel A: All Options
Before
After
DEPENDENT VARIABLE
...
0.043
0.059
As another robustness result, this table reevaluates our Hypothesis 1 and 2 for the sample before and after
the policy change using the same regression as in Table 5, but with the imbalance variable substituted by
a dummy variable (with +1 indicating net buying pressure from clients, 0 indicating a balanced market,
and −1 indicating net selling pressure from clients). Robust t-statistics are in parentheses with standard
errors clustered at the contract level. Stars denotes p-statistics < 0.01, < 0.05, and < 0.1, respectively.
47
This preprint research paper has not been peer reviewed. Electronic copy available at: https://ssrn.com/abstract=4496644
Preprint not peer reviewed
