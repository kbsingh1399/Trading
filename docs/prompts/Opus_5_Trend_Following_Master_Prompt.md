...
## 2. Orderflow & Footprint Data Schema Available in Master Parquets
...
1. Breakout Ignition via Stacked Imbalances:
   - A price breakout of the 20/55-bar Donchian channel or EMA ribbon (EMA_20 > EMA_50 > EMA_200) is ONLY taken if confirmed by orderflow aggression:
     is_stacked_buy_imb == True OR (taker_volume_ratio >= 1.25 AND future_cvd_15m > 0)
...
2. CVD Acceleration & Divergence Confirmation:
   - Long breakouts must show expanding CVD (CVD z-score >= 1.2).
...
3. Liquidation Squeeze Rocket Fuel:
   - When a long trend breakout coincides with elevated short liquidations (short_liq_zs >= 1.5), aggressive forced buy-ins trigger a convexity rocket. These setups are tagged for extended targets (+6.0R to +8.0R).

4. Anti-Suffocation Ratchets (The Critical Lesson from Round 9):
   - In our Round 9 real-data run, 7 trades reached MFE +1.5R to +3.07R, but were stopped out at +0.60R because the trailing stop locked too close.
   - Wide-Breathing Ratchet Schedule:
     - Gain >= +1.0R: Move stop to Breakeven +0.10R.
     - Gain >= +2.0R: Move stop to Entry +1.00R.
     - Gain >= +3.2R: Move stop to Entry +2.20R.
     - Chandelier Trailing Stop: Once past +3.5R, trail at 2.2 * ATR_14 from running highs, allowing 5R-8R mega-runners to mature.

5. Pyramiding & House Money:
   - Add 0.50x pyramid leg once stop is locked at BE; scale risk with banked profit (25 + 0.40 * banked, capped at 200 USD); protect 4,775 USD hard floor (4.5% MaxDD limit).
