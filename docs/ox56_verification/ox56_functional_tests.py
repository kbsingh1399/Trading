...
Target: origin/main @ d4e1433 (via /tmp/ox55_verify sandbox, updated).
...
try:
    FE.MT5Connection().connect()
    check("H.eng-connect raises AttributeError (RESIDUAL)", False, "unexpectedly clean")
except AttributeError as e:
    check("H.eng-connect raises AttributeError (RESIDUAL)", True, f"{type(e).__name__}")
...
print(f"\nALL {len(RESULTS)} OX56 FUNCTIONAL CHECKS PASSED")
