...
OUTPUT_DIR = REPO / "reports" / "ml_reversal_results"
...
def run_fast_numba_walkforward(all_data: pd.DataFrame, return_trades: bool = False):
    ...
    for w in windows:
        ...
        ridge = LogisticRegression(C=0.02874565597723325, max_iter=200, random_state=42)
        ...
        # Hybrid ensemble: 60% Ridge + 40% LightGBM (Optuna Trial #24641 Champion: 10 Passes, +4,629.28 USD)
        ...
        # In-sample causal monthly density calibration (targeting 42.0 cands/month)
        ...
        for idx in range(len(test_times)):
            ...
            if len(open_positions) < 3:
                ...
                elif current_profit >= 440.0:
                    # Transition risk scaling between 440 USD and 500 USD (Trial #24641 champion: 31.0 USD)
                    risk_amt = 31.0
                ...
                # Reset loss streak on authentic win >= +0.90R (Trial #17241 champion)
                if r_gain >= 0.90:
                    consec_losses = 0
                else:
                    consec_losses += 1
