...
def _atomic_write(pldf: pl.DataFrame, out_file: str):
    """Write Parquet atomically via temp file to prevent corruption on crash."""
    tmp_file = out_file + ".tmp"
    pldf.write_parquet(tmp_file, compression="zstd")
    os.replace(tmp_file, out_file)

def export_all_mt5_non_crypto(output_dir: str = "Forex_Backtesting_Data"):
    ...
    print(f"Total Non-Crypto Assets: {len(target_symbols)} x {len(TIMEFRAMES)} timeframes")
