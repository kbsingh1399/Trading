"""
================================================================================
20 OOS WALK-FORWARD HARNESS — NEURAL LIQUIDATION CASCADE (MLP)
================================================================================
...
Enforces Part 14 Institutional Anti-Lookahead Directives:
  * Zero lookup tables or static result scorecards.
  * For each window k, model is trained exclusively on historical data t < t_start - 72h.
  * Zero test-set snooping or parameter tuning on test partitions.
  * Bar j+1 trailing ratchets, 8 bps taker fee, 10 bps entry / 15 bps exit slippage.
  * Live console output with bar-by-bar mark-to-market drawdown verification.
  * Monte Carlo Simulation to evaluate risk of ruin.
================================================================================
"""
...
from Engine.strategy.neural_liquidation_engine import (
    RiskConfig,
    FrictionConfig,
    RatchetConfig,
    FeatureExtractor,
...
def run_monte_carlo(trades: List[Dict], initial_capital: float, iterations: int = 10000) -> Dict:
    ...
    # Resample trades with replacement for N iterations
    simulated_curves = []
    max_drawdowns = []
    terminal_equities = []
    ruin_count = 0
    ruin_threshold = initial_capital * 0.95 # 5% drawdown is ruin
...
def run_walkforward(
    ...
    print("=" * 80)
    print("INSTITUTIONAL WALK-FORWARD HARNESS — NEURAL LIQUIDATION CASCADE (MLP)")
    print("=" * 80)
    ...
    simulator = CausalSimulator(risk_cfg=risk_cfg, fric_cfg=fric_cfg, ratchet_cfg=ratchet_cfg)
    ...
    for w_idx, win in enumerate(oos_windows[:max_windows], start=1):
        ...
        t_feat = time.time()
        X_train = FeatureExtractor.transform(df_train)
        X_test = FeatureExtractor.transform(df_test)
        
        # Triple barrier labeling on training partition only
        y_train = TripleBarrierLabeler.label(df_train, horizon=24, min_tp_r=2.5, sl_r=1.0)
        
        unique, counts = np.unique(y_train, return_counts=True)
        print(f"y_train distribution: {dict(zip(unique, counts))}")
        
        # Event filter on training set
        train_events = (
            (df_train["long_liq_zs"] > 1.0) |
            (df_train["short_liq_zs"] > 1.0) |
            (df_train["vwap_zscore"].abs() > 1.2) |
            (df_train["rsi_14"] < 32) |
            (df_train["rsi_14"] > 68) |
            (df_train["close"] < df_train["session_vwap"] - 2.0 * df_train["atr_14"]) |
            (df_train["close"] > df_train["session_vwap"] + 2.0 * df_train["atr_14"])
        ).values
        
        # Fit Ensemble model
        t_fit = time.time()
        model = EnsembleLiquidationModel(random_state=42 + w_idx)
        model.fit(X_train, y_train, event_mask=train_events)
        fit_dur = time.time() - t_fit
        
        # Predict test probabilities
        _, p_long, p_short = model.predict_probabilities(X_test)
        
        # Execute causal backtest simulation
        print(f"Max p_long: {np.max(p_long):.4f}, Max p_short: {np.max(p_short):.4f}")
        
        # Strictly test with a real model confidence threshold (0.55 for MLP)
        sim_res = simulator.run(df_test, p_long, p_short, p_threshold=0.55, liq_threshold=1.2)
        ...
        # Target Pass Criteria: ROI > 20.0%, MaxDD < 5.0%, WR > 40.0%, Trades >= 6
        ...
        print(f"  Compute:    Fit {fit_dur:.2f}s | Features {time.time()-t_feat:.2f}s")
        ...
        # Clean memory between windows
        del df_train, df_test, X_train, X_test, y_train, model
        gc.collect()
    ...
    # Run Monte Carlo Simulation on the aggregate walk-forward trades
    run_monte_carlo(all_trades, risk_cfg.initial_capital)
    
    return results

def main():
    parser = argparse.ArgumentParser(description="Walk-Forward Evaluation of Neural Liquidation Engine.")
