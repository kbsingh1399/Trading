"""
================================================================================
RP2 ROUND 11 - REGIME-ADAPTIVE CONVEX BREAKOUT-RECLAIM ENGINE (ML OVERLAY)
================================================================================
...
class R11Config:
    ...
    kelly_frac: float = 0.42               # share of banked profit recycled
    risk_cap_pct: float = 0.020            # hard ceiling on any single trade
    flat_risk_mode: bool = False           # True -> flat `flat_risk_pct`
    flat_risk_pct: float = 0.010
    ...
    # --- viability ----------------------------------------------------------
    max_friction_r: float = 0.22           # friction may not exceed 22% of 1R
    ...
    don_fast: int = 20
    don_slow: int = 55
    disp_atr_mult: float = 1.45            # displacement bar range vs ATR14
    arm_expiry_bars: int = 12              # pullback must occur within N bars
    pullback_min_atr: float = 0.25         # minimum retrace depth
    pullback_max_atr: float = 1.30         # beyond this the break is dead
    ...
    conf_min_score: float = 3.0            # out of 8.0 available
    ...
    hold_max_bars: int = 672               # 7 days
    chop_hold_max_bars: int = 192
...
def aggregate_ladder(ladder: pd.DataFrame, master: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"ladder missing columns: {sorted(missing)}")
...
def load_symbol(
    ...
    if with_ladder and os.path.exists(lpath):
        ...
        try:
            ...
            ladder = ladder[(ladder["open_time_ms"] >= t0) & (ladder["open_time_ms"] <= t1)]
        if len(ladder):
...
# ==========================================================================
def build_features(df: pd.DataFrame, cfg: R11Config) -> Dict[str, np.ndarray]:
    n = len(df)
...
def classify_regime(f: Dict[str, np.ndarray], cfg: R11Config) -> np.ndarray:
...
def scan_candidates(sym: str, f: Dict[str, np.ndarray], reg: np.ndarray,
                    cfg: R11Config, i0: int, i1: int) -> List[Candidate]:
    ...
    # armed state per side
    arm_side = 0
    arm_bar = -1
    arm_level = 0.0     # the broken Donchian level
    arm_ext = 0.0       # impulse extreme
    pb_ext = 0.0        # pullback extreme (swing low for longs)
    pb_trigger = 0.0    # level whose reclaim fires the entry
    pb_seen = False
    ...
    for i in range(lo_i, i1):
        ...
        # ---------- expire a stale arm ----------
        if arm_side != 0 and (i - arm_bar) > cfg.arm_expiry_bars + 24:
            arm_side = 0

        # ---------- try to ARM on bar i-1 ----------
        if arm_side == 0:
            broke_up = (np.isfinite(dhi[i - 1]) and h[i - 1] > dhi[i - 1]
                        and c[i - 1] > dhi[i - 1])
            broke_dn = (np.isfinite(dlo[i - 1]) and l[i - 1] < dlo[i - 1]
                        and c[i - 1] < dlo[i - 1])
            disp = (h[i - 1] - l[i - 1]) >= cfg.disp_atr_mult * a
            if broke_up and disp and f["ribbon_up"][i - 1] > 0:
                arm_side, arm_bar = SIDE_LONG, i - 1
                arm_level, arm_ext = dhi[i - 1], h[i - 1]
                pb_ext, pb_trigger, pb_seen = l[i - 1], h[i - 1], False
            elif broke_dn and disp and f["ribbon_dn"][i - 1] > 0:
                arm_side, arm_bar = SIDE_SHORT, i - 1
                arm_level, arm_ext = dlo[i - 1], l[i - 1]
                pb_ext, pb_trigger, pb_seen = h[i - 1], l[i - 1], False
            continue

        j = i - 1  # last fully closed bar
        if arm_side == SIDE_LONG:
            arm_ext = max(arm_ext, h[j])
            # structure broken -> disarm
            if c[j] < arm_level - 0.25 * a:
                arm_side = 0
                continue
            depth = (arm_ext - l[j]) / a
            if depth >= cfg.pullback_min_atr:
                if not pb_seen or l[j] < pb_ext:
                    pb_ext = l[j]
                    pb_trigger = h[j]
                    pb_seen = True
                else:
                    pb_trigger = max(pb_trigger, h[j]) if l[j] <= pb_ext + 0.15 * a else pb_trigger
            if depth > cfg.pullback_max_atr:
                arm_side = 0
                continue
            if (j - arm_bar) > cfg.arm_expiry_bars:
                # expired: disarm unconditionally. Leaving a pb_seen arm alive
                # past expiry froze it into a dead state that could neither
                # fire nor reset until the +24 stale sweep.
                arm_side = 0
                continue
            if not pb_seen:
                continue
            trig = pb_trigger * (1.0 + cfg.reclaim_buffer_bps / 10_000.0)
            if c[j] > trig and h[j] > pb_trigger:
                fp = footprint_score(f, j, SIDE_LONG)
                ok = _entry_confirmed(f, j, SIDE_LONG, fp, cfg)
                if ok:
                    stop = min(pb_ext, l[j]) - 0.10 * a
                    dist = c[j] - stop
                    da = dist / a
            ...
            arm_ext = min(arm_ext, l[j])
            if c[j] > arm_level + 0.25 * a:
                arm_side = 0
                continue
            depth = (h[j] - arm_ext) / a
            if depth >= cfg.pullback_min_atr:
                if not pb_seen or h[j] > pb_ext:
                    pb_ext = h[j]
                    pb_trigger = l[j]
                    pb_seen = True
                else:
                    pb_trigger = min(pb_trigger, l[j]) if h[j] >= pb_ext - 0.15 * a else pb_trigger
            if depth > cfg.pullback_max_atr:
                arm_side = 0
                continue
            if (j - arm_bar) > cfg.arm_expiry_bars:
            ...
            if not pb_seen:
                continue
            trig = pb_trigger * (1.0 - cfg.reclaim_buffer_bps / 10_000.0)
            if c[j] < trig and l[j] < pb_trigger:
                fp = footprint_score(f, j, SIDE_SHORT)
                ok = _entry_confirmed(f, j, SIDE_SHORT, fp, cfg)
                if ok:
                    stop = max(pb_ext, h[j]) + 0.10 * a
                    dist = stop - c[j]
                    da = dist / a
                    if cfg.min_stop_atr <= da <= cfg.max_stop_atr:
                        out.append(Candidate(
...
class _HistGBDT:
    ...
    def __init__(self, cfg: R11Config):
        self.cfg = cfg
            ...
            if cands and len(open_pos) < cfg.max_concurrent:
                ...
                for _, cd in scored:
                    ...
                    pos = Position(
                        ...
                        stop=cd.stop_px, tp=entry + cd.side * tp_mult * r_unit,
    ...
    for w in windows:
        ...
        for sym in cfg.symbols:
            ...
            if df is None or len(df) < cfg.don_slow + cfg.regime_lookback + 200:
                continue
            of_cov.append(orderflow_cover(df))
            ...
            for cd in scan_candidates(sym, f, reg, cfg, i_ws, i_end):
                if ws <= ts[cd.i] <= we:
...
def run_walkforward_from_json(cfg: R11Config, windows_path: str,
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="RP2 Round 11 walk-forward")
    ...
    c = R11Config(data_dir=a.data_dir, flat_risk_mode=a.flat_risk,
