"""
================================================================================
DUAL-CLASSIFIER LIQUIDATION CASCADE QUANTITATIVE ENGINE (NEURAL MLP)
================================================================================
Institutional quantitative strategy for BTCUSDT (15m timeframe).
Uses an MLP (Multi-Layer Perceptron) architecture to replace tree-based models,
aiming to reduce overfitting and better handle regime shifts.
...
class EnsembleLiquidationModel:
    def __init__(self, random_state: int = 42, **kwargs):
        ...
        # Use Pipeline to scale features before passing to MLP
        mlp_long = MLPClassifier(hidden_layer_sizes=(64, 32), activation='relu', solver='adam', alpha=0.01, 
                                 batch_size=256, learning_rate='adaptive', max_iter=200, early_stopping=True, 
                                 n_iter_no_change=10, random_state=random_state)
        mlp_short = MLPClassifier(hidden_layer_sizes=(64, 32), activation='relu', solver='adam', alpha=0.01, 
                                  batch_size=256, learning_rate='adaptive', max_iter=200, early_stopping=True, 
                                  n_iter_no_change=10, random_state=random_state)
                                  
        self.pipeline_long = Pipeline([
            ('scaler', StandardScaler()),
            ('mlp', mlp_long)
        ])
        
        self.pipeline_short = Pipeline([
            ('scaler', StandardScaler()),
            ('mlp', mlp_short)
        ])
        
    def fit(self, X: pd.DataFrame, y: np.ndarray, event_mask: Optional[np.ndarray] = None) -> EnsembleLiquidationModel:
        print("Training Dual Neural Classifiers (MLP Long/Short)...")
        ...
        if y_long.sum() > 5 and (y_long == 0).sum() > 5:
            self.pipeline_long.fit(X_train, y_long)
        else:
            print("Warning: Not enough positive/negative samples for LONG classifier.")
            
        if y_short.sum() > 5 and (y_short == 0).sum() > 5:
            self.pipeline_short.fit(X_train, y_short)
        else:
            print("Warning: Not enough positive/negative samples for SHORT classifier.")
    ...
    def predict_probabilities(self, X: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Check if the pipeline's estimator is fitted
        if hasattr(self.pipeline_long.named_steps['mlp'], "classes_"):
            p_long = self.pipeline_long.predict_proba(X_val)[:, 1]
             
        if hasattr(self.pipeline_short.named_steps['mlp'], "classes_"):
            p_short = self.pipeline_short.predict_proba(X_val)[:, 1]
    ...
    def run(self, df_test: pd.DataFrame, p_long: np.ndarray, p_short: np.ndarray, p_threshold: float = 0.55, liq_threshold: float = 1.8) -> Dict:
        ...
        for t in range(self.ratchet.time_decay_bars, T):
            ...
            if pos_side == 0 and t < T - 1:
                ...
                # MLPs might output slightly different probability scales, so adjust threshold
