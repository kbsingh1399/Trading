"""
================================================================================
20 OOS WALK-FORWARD HARNESS â€” ML OVERLAY CLASSIFIER
================================================================================
...
from Engine.strategy.voting_liquidation_engine import (
    RiskConfig,
    FrictionConfig,
    RatchetConfig,
    FeatureExtractor,
    TripleBarrierLabeler,
    VotingLiquidationModel,
    CausalVotingSimulator,
)
...
def run_walkforward(
    ...
    print("=" * 80)
    print("INSTITUTIONAL WALK-FORWARD HARNESS â€” VOTING ENGINE")
    print("=" * 80)
    ...
    simulator = CausalVotingSimulator(risk_cfg=risk_cfg, fric_cfg=fric_cfg, ratchet_cfg=ratchet_cfg)
    ...
    for w_idx, win in enumerate(oos_windows[:max_windows], start=1):
        ...
        # Compute candidate event mask to train only on interesting volatility/liquidity events
        long_liq_zs = df_train["long_liq_zs"].fillna(0.0).values
        short_liq_zs = df_train["short_liq_zs"].fillna(0.0).values
        vwap_zscore = df_train["vwap_zscore"].fillna(0.0).values
        rsi_14 = df_train["rsi_14"].fillna(50.0).values
        
        base_filter_long = (long_liq_zs > 0.5) | (vwap_zscore < -0.5) | (rsi_14 < 45)
        base_filter_short = (short_liq_zs > 0.5) | (vwap_zscore > 0.5) | (rsi_14 > 55)
        event_mask = base_filter_long | base_filter_short
        
        print(f"Overlay training samples: {event_mask.sum()} (Long: {(y_train[event_mask] == 1).sum()}, Short: {(y_train[event_mask] == -1).sum()})")
        
        # Train ML overlay dynamically based on candidate signals identified within the training partition
        model = VotingLiquidationModel(random_state=42 + w_idx)
        t_fit = time.time()
        model.fit(X_train, y_train, event_mask=event_mask)
        ...
        print(f"Max p_long: {p_long.max():.4f}, Max p_short: {p_short.max():.4f}")
        print(f"p_long > {ml_threshold}: {(p_long >= ml_threshold).sum()}, p_short > {ml_threshold}: {(p_short >= ml_threshold).sum()}")
        
        sim_res = simulator.run(df_test, p_long, p_short, class_threshold=ml_threshold)
...
def main():
    parser = argparse.ArgumentParser(description="Walk-Forward Evaluation of Voting Liquidation Engine.")
    ...
    parser.add_argument("--ml-threshold", type=float, default=0.40,
                        help="Voting ensemble probability threshold (default 0.40)")
